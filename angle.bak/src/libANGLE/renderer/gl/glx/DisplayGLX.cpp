...
egl::Error DisplayGLX::initialize(egl::Display *display)
{
    ...
    else
    {
        ...
        // Since our whole EGL implementation is backed by only one FGL context, this
    ...
    else
    {
        ...
        if (!mContext)
        {
            return egl::Error(EGL_NOT_INITIALIZED, "Could not create FGL context.");
        }
    }
    ASSERT(mContext);


    // FunctionsGL and DisplayGL need to make a few FGL calls, for example to
                                         ...
                                         glx::Context *context)
{
    ...
    // FGL version until we succeed, starting from newer version. When the default
