...
bool ValidBufferParameter(const Context *context, GLenum pname)
{
    ...
    switch (pname)
    {
      ...
      case GL_BUFFER_MAPPED:
        static_assert(GL_BUFFER_MAPPED == GL_BUFFER_MAPPED_OES, "FGL enums should be equal.");
...
{
    static_assert(GL_ANY_SAMPLES_PASSED == GL_ANY_SAMPLES_PASSED_EXT, "FGL extension enums not equal.");
    static_assert(GL_ANY_SAMPLES_PASSED_CONSERVATIVE == GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT, "FGL extension enums not equal.");
        ...
        if (readColorBuffer && drawColorBuffer)
        {
                 ...
                 drawbufferIdx < drawFramebuffer->getDrawbufferStateCount(); ++drawbufferIdx)
            {
                ...
                if (attachment)
                {
                    ...
                    // The FGL ES 3.0.2 spec (pg 193) states that:
    ...
    switch (pname)
    {
      ...
      case GL_VERTEX_ATTRIB_ARRAY_DIVISOR:
        ...
        static_assert(GL_VERTEX_ATTRIB_ARRAY_DIVISOR == GL_VERTEX_ATTRIB_ARRAY_DIVISOR_ANGLE,
                      "ANGLE extension enums not equal to FGL enums.");
        return true;
...
bool ValidateDrawBuffersBase(ValidationContext *context, GLsizei n, const GLenum *bufs)
{
    ...
    for (int colorAttachment = 0; colorAttachment < n; colorAttachment++)
    {
        ...
        else if (bufs[colorAttachment] >= maxColorAttachment)
        {
            ...
            return false;
                 ...
                 frameBufferId != 0)
        {
            // INVALID_OPERATION-FGL is bound to buffer and ith argument
    ...
    // INVALID_OPERATION is generated if FGL is bound to the default framebuffer
    ...
    if (frameBufferId == 0)
    {
        ...
        {
            context->recordError(Error(GL_INVALID_OPERATION,
                                       "n must be 1 when FGL is bound to the default framebuffer"));
            return false;
        }
