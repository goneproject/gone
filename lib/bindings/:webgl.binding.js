...
let HEADLESS_VERSION = require('../../package.json').version;

// These are defined by the WebGL spec
let MAX_UNIFORM_LENGTH = 256;
let MAX_ATTRIBUTE_LENGTH = 256;

// We need to wrap some of the native WebGL functions to handle certain error codes and check input values
let gl = nativeGL.WebGLRenderingContext.prototype;
gl.VERSION = 0x1F02;
gl.IMPLEMENTATION_COLOR_READ_TYPE = 0x8B9A;
gl.IMPLEMENTATION_COLOR_READ_FORMAT = 0x8B9B;

let ATTACHMENTS = [gl.COLOR_ATTACHMENT0, gl.DEPTH_ATTACHMENT, gl.STENCIL_ATTACHMENT, gl.DEPTH_STENCIL_ATTACHMENT];

// Hook clean up
process.on('exit', nativeGL.cleanup);

// Export type boxes for WebGL
let WebGLRenderingContext = nativeGL.WebGLRenderingContext;
...
function WebGLContextAttributes(alpha, depth, stencil, antialias, premultipliedAlpha, preserveDrawingBuffer, preferLowPowerToHighPerformance, failIfMajorPerformanceCaveat) {
  this.alpha = alpha;
  this.depth = depth;
...
function OES_texture_float(input) {
  return input;
}
...
function activeTexture(context, target) {
  let activeUnit = activeTextureUnit(context);
...
function precheckFramebufferStatus(framebuffer) {
  let attachments = framebuffer._attachments;
  let width = [];
  let height = [];

  let colorAttachment = attachments[gl.COLOR_ATTACHMENT0];
  let depthAttachment = attachments[gl.DEPTH_ATTACHMENT];
  let depthStencilAttachment = attachments[gl.DEPTH_STENCIL_ATTACHMENT];
  let stencilAttachment = attachments[gl.STENCIL_ATTACHMENT];
  ...
  if (colorAttachment instanceof WebGLTexture) {
    if (colorAttachment._format !== gl.RGBA || colorAttachment._type !== gl.UNSIGNED_BYTE) {
      return gl.FRAMEBUFFER_INCOMPLETE_ATTACHMENT;
    }
    let level = framebuffer._attachmentLevel[gl.COLOR_ATTACHMENT0];
    ...
    let format = colorAttachment._format;
  ...
  if (!colorAttachment && !stencilAttachment && !depthAttachment && !depthStencilAttachment) {
    return gl.FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT;
  }
...
function framebufferOk(context) {
  let framebuffer = context._activeFramebuffer;
  if (framebuffer && precheckFramebufferStatus(framebuffer) !== gl.FRAMEBUFFER_COMPLETE) {
...
function getTexImage(context, target) {
  let unit = activeTextureUnit(context);
  if (target === gl.TEXTURE_2D) {
    return unit._bind2D;
...
function validFramebufferAttachment(attachment) {
  return attachment === gl.COLOR_ATTACHMENT0 || attachment === gl.DEPTH_ATTACHMENT || attachment === gl.STENCIL_ATTACHMENT || attachment === gl.DEPTH_STENCIL_ATTACHMENT;
}
...
function checkLocation(context, location) {
  if (!(location instanceof WebGLUniformLocation)) {
    ...
    return false;
  } else if (location._program._ctx !== context || location._linkCount !== location._program._linkCount) {
    setError(context, gl.INVALID_OPERATION);
    return false;
  }
  return true;
...
function restoreError(context, lastError) {
  let topError = context._errorStack.pop();
...
function checkVertexAttribState(context, maxIndex) {
  let program = context._activeProgram;
  if (!program) {
    setError(context, gl.INVALID_OPERATION);
    return false;
  }
  let attribs = context._vertexAttribs;
  for (let i = 0; i < attribs.length; ++i) {
    let attrib = attribs[i];
    if (attrib._isPointer) {
      let buffer = attrib._pointerBuffer;
      if (!buffer) {
        setError(context, gl.INVALID_OPERATION);
        return false;
      }
...
function clearFramebufferAttachment(framebuffer, attachment) {
  let object = framebuffer._attachments[attachment];
  if (!object) {
    return;
  }
...
function setFramebufferAttachment(framebuffer, object, attachment) {
  let prevObject = framebuffer._attachments[attachment];
...
function createANGLEInstancedArrays(context) {
  ...
  let result = new ANGLE_instanced_arrays();
  ...
  result.drawArraysInstancedANGLE = function (mode, first, count, primCount) {
    ...
    let reducedCount = vertexCount(mode, count);
    if (reducedCount < 0) {
      setError(context, gl.INVALID_ENUM);
      return;
    }
    ...
    if (checkInstancedVertexAttribState(context, maxIndex, primCount)) {
      return _drawArraysInstanced.call(context, mode, first, reducedCount, primCount);
    }
  };
  result.drawElementsInstancedANGLE = function (mode, count, type, ioffset, primCount) {
    mode |= 0;
    count |= 0;
    type |= 0;
    ioffset |= 0;
    ...
    let elementBuffer = context._activeElementArrayBuffer;
    if (!elementBuffer) {
      setError(context, gl.INVALID_OPERATION);
      return;
    }
  ...
  result.vertexAttribDivisorANGLE = function (index, divisor) {
    ...
    let attrib = context._vertexAttribs[index];
...
gl.isExtensionSupportedx = function isExtensionSupported(name) {
  return this.isExtensionSupported("OES_texture_float");
};

gl.getExtensionx = function getExtension(name) {
  ...
  switch (str) {
    ...
    case 'oes_texture_float':
      ext = new OES_texture_float(this);
      break;
  }
  if (ext) {
...
let _activeTexture = gl.activeTexture;
gl.activeTexture = function activeTexture(texture) {
  texture |= 0;
  let texNum = texture - gl.TEXTURE0;
...
let _attachShader = gl.attachShader;
gl.attachShader = function attachShader(program, shader) {
  ...
  if (!program || !shader) {
    ...
    return;
  } else if (program instanceof WebGLProgram && shader instanceof WebGLShader && checkOwns(this, program) && checkOwns(this, shader)) {
...
let _bindAttribLocation = gl.bindAttribLocation;
...
let _bindRenderbuffer = gl.bindRenderbuffer;
gl.bindRenderbuffer = function (target, object) {
  ...
  let active = this._activeRenderbuffer;
  if (active !== object) {
    if (active) {
      active._refCount -= 1;
      checkDelete(active);
...
let _bindFramebuffer = gl.bindFramebuffer;
gl.bindFramebuffer = function bindFramebuffer(target, framebuffer) {
  ...
  if (!framebuffer) {
    _bindFramebuffer.call(this, gl.FRAMEBUFFER, this._drawingBuffer._framebuffer);
  ...
  let activeFramebuffer = this._activeFramebuffer;
...
let _bindTexture = gl.bindTexture;
gl.bindTexture = function bindTexture(target, texture) {
  ...
  let activeUnit = activeTextureUnit(this);
  let activeTex = activeTexture(this, target);
...
let _blendEquation = gl.blendEquation;
...
let _blendEquationSeparate = gl.blendEquationSeparate;
...
let _blendFuncSeparate = gl.blendFuncSeparate;
gl.blendFuncSeparate = function blendFuncSeparate(srcRGB, dstRGB, srcAlpha, dstAlpha) {
  ...
  if (!(validBlendFunc(srcRGB) && validBlendFunc(dstRGB) && validBlendFunc(srcAlpha) && validBlendFunc(dstAlpha))) {
    setError(this, gl.INVALID_ENUM);
    return;
  }

  if (isConstantBlendFunc(srcRGB) && isConstantBlendFunc(dstRGB) || isConstantBlendFunc(srcAlpha) && isConstantBlendFunc(dstAlpha)) {
    setError(this, gl.INVALID_OPERATION);
    return;
  }
...
gl.bufferData = function bufferData(target, data, usage) {
  ...
  let active = getActiveBuffer(this, target);
  if (!active) {
    setError(this, gl.INVALID_OPERATION);
    return;
  }
...
let _bufferSubData = gl.bufferSubData;
...
//   }
//
//   let framebuffer = this._activeFramebuffer
//   if (!framebuffer) {
//     return gl.FRAMEBUFFER_COMPLETE
//   }
//
  ...
  for (let i = 0; i < tokens.length; ++i) {
    ...
    switch (tok.type) {
      case 'ident':
        ...
        break;
      case 'preprocessor':
        let bodyToks = tokenize(tok.data.match(/^\s*#\s*(.*)$/)[1]);
...
gl.compileShader = function compileShader(shader) {
  ...
  if (checkWrapper(this, shader, WebGLShader) && checkShaderSource(this, shader)) {
    let prevError = this.getError();
    _compileShader.call(this, shader._ | 0);
    let error = this.getError();
    shader._compileStatus = !!_getShaderParameter.call(this, shader._ | 0, gl.COMPILE_STATUS);
...
let _copyTexImage2D = gl.copyTexImage2D;
gl.copyTexImage2D = function copyTexImage2D(target, level, internalformat, x, y, width, height, border) {
  ...
  let texture = getTexImage(this, target);
  if (!texture) {
    setError(this, gl.INVALID_OPERATION);
    return;
  }
  ...
  saveError(this);
  _copyTexImage2D.call(this, target, level, internalformat, x, y, width, height, border);
  let error = this.getError();
  restoreError(this, error);
...
let _copyTexSubImage2D = gl.copyTexSubImage2D;
gl.copyTexSubImage2D = function copyTexSubImage2D(target, level, xoffset, yoffset, x, y, width, height) {
  ...
  _copyTexSubImage2D.call(this, target, level, xoffset, yoffset, x, y, width, height);
};

let _cullFace = gl.cullFace;
...
function createObject(method, Wrapper, refset) {
  let native = gl[method];
  gl[method] = function () {
    let id = native.call(this);
    if (id <= 0) {
      return null;
    } else {
      let result = new Wrapper(id, this);
...
let _createFramebuffer = gl.createFramebuffer;
let _createRenderbuffer = gl.createRenderbuffer;
let _createTexture = gl.createTexture;
...
let _createShader = gl.createShader;
gl.createShader = function (type) {
  ...
  let id = _createShader.call(this, type);
  if (id < 0) {
    return null;
  }
  let result = new WebGLShader(id, this, type);
...
let _deleteBuffer = gl.deleteBuffer;
...
gl.deleteBuffer = function deleteBuffer(buffer) {
  ...
  for (let i = 0; i < this._vertexAttribs.length; ++i) {
    let attrib = this._vertexAttribs[i];
...
let _deleteFramebuffer = gl.deleteFramebuffer;
...
gl.deleteFramebuffer = function deleteFramebuffer(framebuffer) {
  ...
  if (!(framebuffer instanceof WebGLFramebuffer && checkOwns(this, framebuffer))) {
    setError(this, gl.INVALID_OPERATION);
    return;
  }
...
//
let _deleteRenderbuffer = gl.deleteRenderbuffer;
...
gl.deleteRenderbuffer = function deleteRenderbuffer(renderbuffer) {
  ...
  if (!(renderbuffer instanceof WebGLRenderbuffer && checkOwns(this, renderbuffer))) {
    setError(this, gl.INVALID_OPERATION);
    return;
  }
  ...
  function tryDetach(framebuffer) {
    if (framebuffer && linked(framebuffer, renderbuffer)) {
      let attachments = Object.keys(framebuffer._attachments);
      for (let i = 0; i < attachments.length; ++i) {
        if (framebuffer._attachments[ATTACHMENTS[i]] === renderbuffer) {
          ctx.framebufferTexture2D(gl.FRAMEBUFFER, ATTACHMENTS[i] | 0, gl.TEXTURE_2D, null);
        }
      }
    }
  }
...
let _deleteTexture = gl.deleteTexture;
...
gl.deleteTexture = function deleteTexture(texture) {
  ...
  let curActive = this._activeTextureUnit;

  for (let i = 0; i < this._textureUnits.length; ++i) {
    let unit = this._textureUnits[i];
...
let _detachShader = gl.detachShader;
gl.detachShader = function detachShader(program, shader) {
  if (!checkObject(program) || !checkObject(shader)) {
    throw new TypeError('detachShader(WebGLProgram, WebGLShader)');
  }
  if (checkWrapper(this, program, WebGLProgram) && checkWrapper(this, shader, WebGLShader)) {
    if (linked(program, shader)) {
...
let _disableVertexAttribArray = gl.disableVertexAttribArray;
...
let _vertexAttribDivisor = gl.vertexAttribDivisor;
...
function beginAttrib0Hack(context) {
  ...
  _bufferData.call(context, gl.ARRAY_BUFFER, context._vertexAttribs[0]._data, gl.STREAM_DRAW);
...
function endAttrib0Hack(context) {
  let attrib = context._vertexAttribs[0];
  ...
  _vertexAttribPointer.call(context, 0, attrib._inputSize, attrib._pointerType, attrib._pointerNormal, attrib._inputStride, attrib._pointerOffset);
...
function checkStencilState(context) {
  if (context.getParameter(gl.STENCIL_WRITEMASK) !== context.getParameter(gl.STENCIL_BACK_WRITEMASK) || context.getParameter(gl.STENCIL_VALUE_MASK) !== context.getParameter(gl.STENCIL_BACK_VALUE_MASK) || context.getParameter(gl.STENCIL_REF) !== context.getParameter(gl.STENCIL_BACK_REF)) {
    setError(context, gl.INVALID_OPERATION);
    return false;
  }
  return true;
...
let _drawArrays = gl.drawArrays;
let _drawArraysInstanced = gl.drawArraysInstanced;
...
let _drawElements = gl.drawElements;
let _drawElementsInstanced = gl.drawElementsInstanced;
...
gl.drawElements = function drawElements(mode, count, type, ioffset) {
  ...
  let elementBuffer = this._activeElementArrayBuffer;
  if (!elementBuffer) {
    setError(this, gl.INVALID_OPERATION);
    return;
  }
...
let _enable = gl.enable;
gl.enable = function enable(cap) {
  cap |= 0;
  _enable.call(this, cap);
};

let _enableVertexAttribArray = gl.enableVertexAttribArray;
...
function updateFramebufferAttachments(framebuffer) {
  let prevStatus = framebuffer._status;
  let ctx = framebuffer._ctx;
  let i;
  let attachmentEnum;
  ...
  if (framebuffer._status !== gl.FRAMEBUFFER_COMPLETE) {
    if (prevStatus === gl.FRAMEBUFFER_COMPLETE) {
      for (i = 0; i < ATTACHMENTS.length; ++i) {
        ...
        _framebufferTexture2D.call(ctx, gl.FRAMEBUFFER, attachmentEnum, framebuffer._attachmentFace[attachmentEnum], 0, framebuffer._attachmentLevel[attachmentEnum]);
      }
    }
    return;
  }
  ...
  for (i = 0; i < ATTACHMENTS.length; ++i) {
    attachmentEnum = ATTACHMENTS[i];
    let attachment = framebuffer._attachments[attachmentEnum];
    if (attachment instanceof WebGLTexture) {
      _framebufferTexture2D.call(ctx, gl.FRAMEBUFFER, attachmentEnum, framebuffer._attachmentFace[attachmentEnum], attachment._ | 0, framebuffer._attachmentLevel[attachmentEnum]);
    } else if (attachment instanceof WebGLRenderbuffer) {
      _framebufferRenderbuffer.call(ctx, gl.FRAMEBUFFER, attachmentEnum, gl.RENDERBUFFER, attachment._ | 0);
    }
  }
}

let _framebufferRenderbuffer = gl.framebufferRenderbuffer;
gl.framebufferRenderbuffer = function framebufferRenderbuffer(target, attachment, renderbuffertarget, renderbuffer) {
  ...
  if (target !== gl.FRAMEBUFFER || !validFramebufferAttachment(attachment) || renderbuffertarget !== gl.RENDERBUFFER) {
    setError(this, gl.INVALID_ENUM);
    return;
  }
...
let _framebufferTexture2D = gl.framebufferTexture2D;
gl.framebufferTexture2D = function framebufferTexture2D(target, attachment, textarget, texture, level) {
  ...
  if (target !== gl.FRAMEBUFFER || !validFramebufferAttachment(attachment)) {
    setError(this, gl.INVALID_ENUM);
    return;
  }
...
gl.getActiveAttrib = function getActiveAttrib(program, index) {
  if (!checkObject(program)) {
    ...
    let info = _getActiveAttrib.call(this, program._ | 0, index | 0);
    if (info) {
...
let _getActiveUniform = gl.getActiveUniform;
gl.getActiveUniform = function getActiveUniform(program, index) {
  if (!checkObject(program)) {
    ...
    let info = _getActiveUniform.call(this, program._ | 0, index | 0);
    if (info) {
      return new WebGLActiveInfo(info);
    }
  }
...
let _getAttachedShaders = gl.getAttachedShaders;
gl.getAttachedShaders = function getAttachedShaders(program) {
  ...
  if (!program) {
    ...
    let shaderArray = _getAttachedShaders.call(this, program._ | 0);
    if (!shaderArray) {
      return null;
    }
    let unboxedShaders = new Array(shaderArray.length);
...
let _getAttribLocation = gl.getAttribLocation;
...
let _getParameter = gl.getParameter;
...
let _getShaderPrecisionFormat = gl.getShaderPrecisionFormat;
gl.getShaderPrecisionFormat = function getShaderPrecisionFormat(shaderType, precisionType) {
  shaderType |= 0;
  precisionType |= 0;
  ...
  let format = _getShaderPrecisionFormat.call(this, shaderType, precisionType);
  if (!format) {
    return null;
  }
...
let _getBufferParameter = gl.getBufferParameter;
  ...
  if (object === null) {
    ...
    switch (pname) {
      ...
      case gl.FRAMEBUFFER_ATTACHMENT_TEXTURE_CUBE_MAP_FACE:
        let face = framebuffer._attachmentFace[attachment];
...
let _getProgramParameter = gl.getProgramParameter;
...
let _getProgramInfoLog = gl.getProgramInfoLog;
...
let _getRenderbufferParameter = gl.getRenderbufferParameter;
gl.getRenderbufferParameter = function getRenderbufferParameter(target, pname) {
  ...
  let renderbuffer = this._activeRenderbuffer;
  if (!renderbuffer) {
    setError(this, gl.INVALID_OPERATION);
    return null;
  }
...
let _getShaderParameter = gl.getShaderParameter;
...
let _getShaderInfoLog = gl.getShaderInfoLog;
...
let _getTexParameter = gl.getTexParameter;
gl.getTexParameter = function getTexParameter(target, pname) {
  ...
  let unit = activeTextureUnit(this);
...
gl.getUniform = function getUniform(program, location) {
  if (!checkObject(program) || !checkObject(location)) {
    ...
    let data = _getUniform.call(this, program._ | 0, location._ | 0);
    if (!data) {
      return null;
    }
...
let _getUniformLocation = gl.getUniformLocation;
gl.getUniformLocation = function getUniformLocation(program, name) {
  ...
  if (checkWrapper(this, program, WebGLProgram)) {
    let loc = _getUniformLocation.call(this, program._ | 0, name);
    if (loc >= 0) {
      let searchName = name;
      if (/\[\d+\]$/.test(name)) {
      ...
      for (let i = 0; i < program._uniforms.length; ++i) {
        let infoItem = program._uniforms[i];
      ...
      let result = new WebGLUniformLocation(loc, program, info);

      // handle array case
      if (/\[0\]$/.test(name)) {
        let baseName = name.replace(/\[0\]$/, '');
        let arrayLocs = [];
        ...
        for (i = 0; this.getError() === gl.NO_ERROR; ++i) {
          let xloc = _getUniformLocation.call(this, program._ | 0, baseName + '[' + i + ']');
...
gl.getVertexAttrib = function getVertexAttrib(index, pname) {
  ...
  let attrib = this._vertexAttribs[index];

  let extInstancing = this._extensions.angle_instanced_arrays;
  if (extInstancing) {
...
let _linkProgram = gl.linkProgram;
...
function fixupLink(context, program) {
  ...
  let numAttribs = context.getProgramParameter(program, gl.ACTIVE_ATTRIBUTES);
  let names = new Array(numAttribs);
  ...
  for (let i = 0; i < numAttribs; ++i) {
    _bindAttribLocation.call(context, program._ | 0, program._attributes[i], names[i]);
  }

  _linkProgram.call(context, program._ | 0);

  let numUniforms = context.getProgramParameter(program, gl.ACTIVE_UNIFORMS);
...
let _pixelStorei = gl.pixelStorei;
...
gl.readPixels = function readPixels(x, y, width, height, format, type, pixels) {
  ...
  let imageSize = rowStride * (height - 1) + width * 4;
  if (imageSize <= 0) {
    return;
  }
  ...
  let pixelData = unpackTypedArray(pixels);
  ...
  if (x >= viewWidth || x + width <= 0 || y >= viewHeight || y + height <= 0) {
    ...
    if (nwidth > 0 && nheight > 0) {
      let subPixels = new Uint8Array(nRowStride * nheight);
      _readPixels.call(this, nx, ny, nwidth, nheight, format, type, subPixels);

      let offset = 4 * (nx - x) + (ny - y) * rowStride;
...
let _renderbufferStorage = gl.renderbufferStorage;
...
let _shaderSource = gl.shaderSource;
...
function computePixelSize(context, type, internalformat) {
  let pixelSize = formatSize(internalformat);
  if (pixelSize === 0) {
    setError(context, gl.INVALID_ENUM);
    return 0;
  }
...
function checkDimensions(context, target, width, height, level) {
  ...
  if (target === gl.TEXTURE_2D) {
    if (width > context._maxTextureSize || height > context._maxTextureSize || level > context._maxTextureLevel) {
      setError(context, gl.INVALID_VALUE);
      return false;
    }
  } else if (validCubeTarget(target)) {
    if (width > context._maxCubeMapSize || height > context._maxCubeMapSize || level > context._maxCubeMapLevel) {
      setError(context, gl.INVALID_VALUE);
      return false;
    }
  } else {
...
function convertPixels(pixels) {
  if (typeof pixels === 'object' && pixels !== null) {
    ...
    } else if (pixels instanceof Uint8Array || pixels instanceof Uint16Array || pixels instanceof Uint8ClampedArray) {
...
gl.texImage2D = function texImage2D(target, level, internalformat, width, height, border, format, type, pixels) {
  ...
  let pixelSize = computePixelSize(this, type, format);
  if (pixelSize === 0) {
    return;
  }
  ...
  let rowStride = computeRowStride(this, width, pixelSize);
  ...
  saveError(this);
  _texImage2D.call(this, target, level, internalformat, width, height, border, format, type, data);
  let error = this.getError();
  restoreError(this, error);
  if (error !== gl.NO_ERROR) {
    return;
...
let _texSubImage2D = gl.texSubImage2D;
gl.texSubImage2D = function texSubImage2D(target, level, xoffset, yoffset, width, height, format, type, pixels) {
  ...
  _texSubImage2D.call(this, target, level, xoffset, yoffset, width, height, format, type, data);
};

let _texParameterf = gl.texParameterf;
...
let _texParameteri = gl.texParameteri;
...
function makeUniforms() {
  function makeMatrix(i) {
    ...
    gl[func] = function (location, transpose, v) {
      ...
      } else if (!!transpose || typeof v !== 'object' || v === null || !v.length || v.length % i * i !== 0) {
        setError(this, gl.INVALID_VALUE);
        return;
      }
      if (!location) {
      ...
      if (v.length === i * i) {
        ...
        let arrayLocs = location._array;
        for (let j = 0; j < arrayLocs.length && (j + 1) * i * i <= v.length; ++j) {
          native.call(this, arrayLocs[j], !!transpose, new Float32Array(data.subarray(j * i * i, (j + 1) * i * i)));
        }
      } else {
        setError(this, gl.INVALID_VALUE);
      }
  ...
  for (let n = 1; n <= 4; ++n) {
    ...
    ['i', 'f'].forEach(function (type) {
      let i = n;
      let func = 'uniform' + i + type;
      let native = gl[func];

      let base = gl[func] = function (location, x, y, z, w) {
        if (!checkObject(location)) {
          throw new TypeError(func + '(WebGLUniformLocation, ...)');
        } else if (!location) {} else if (checkLocationActive(this, location)) {
          let utype = location._activeInfo.type;
...
let _validateProgram = gl.validateProgram;
...
function makeVertexAttribs() {
  function makeVertex(i) {
    let func = 'vertexAttrib' + i + 'f';
    let native = gl[func];

    let base = gl[func] = function (idx, x, y, z, w) {
      idx |= 0;
      if (idx < 0 || idx >= this._vertexAttribs.length) {
        setError(this, gl.INVALID_VALUE);
        return;
      }
      let data = this._vertexAttribs[idx]._data;
      data[3] = 1;
...
let _vertexAttribPointer = gl.vertexAttribPointer;
...
function allocateDrawingBuffer(context, width, height) {
  context._drawingBuffer = new WebGLDrawingBufferWrapper(_createFramebuffer.call(context), _createTexture.call(context), _createRenderbuffer.call(context));

  resizeDrawingBuffer(context, width, height);
}

function resizeDrawingBuffer(context, width, height) {
  let prevFramebuffer = context._activeFramebuffer;
  let prevTexture = activeTexture(context, gl.TEXTURE_2D);
  let prevRenderbuffer = context._activeRenderbuffer;

  let contextAttributes = context._contextattributes;

  let drawingBuffer = context._drawingBuffer;
  _bindFramebuffer.call(context, gl.FRAMEBUFFER, drawingBuffer._framebuffer);

  // Clear all attachments
  for (let i = 0; i < ATTACHMENTS.length; ++i) {
    _framebufferTexture2D.call(context, gl.FRAMEBUFFER, ATTACHMENTS[i], gl.TEXTURE_2D, 0, 0);
  }

  // Update color attachment
  _bindTexture.call(context, gl.TEXTURE_2D, drawingBuffer._color);
  let colorFormat = contextAttributes.alpha ? gl.RGBA : gl.RGB;
  _texImage2D.call(context, gl.TEXTURE_2D, 0, colorFormat, width, height, 0, colorFormat, gl.UNSIGNED_BYTE, null);
  ...
  _framebufferTexture2D.call(context, gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, drawingBuffer._color, 0);
  ...
  if (storage) {
    _bindRenderbuffer.call(context, gl.RENDERBUFFER, drawingBuffer._depthStencil);
    _renderbufferStorage.call(context, gl.RENDERBUFFER, storage, width, height);
    _framebufferRenderbuffer.call(context, gl.FRAMEBUFFER, attachment, gl.RENDERBUFFER, drawingBuffer._depthStencil);
  }
...
module.exports = {
  ...
  allocateDrawingBuffer: allocateDrawingBuffer
};