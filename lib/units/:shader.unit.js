...
let exec = async (gl, ipt, code) => {
    ...
    let vertexShaderCode = "attribute vec2 position;\n" + "varying vec2 pos;\n" + "attribute vec2 texture;\n" + "\n" + "void main(void) {\n" + "  pos = texture;\n" + "  gl_Position = vec4(position.xy, 0.0, 1.0);\n" + "}";
    ...
    if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
        ...
        for (let nl = 0; nl < LOC.length; nl++) dbgMsg += stdlib.split('\n').length + nl + "> " + LOC[nl] + "\n";
...
let alloc = sz => {
    ...
    if (sz > 16777216) throw new Error("ENGINE: Whoops, the maximum array size is exceeded!");
