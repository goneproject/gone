...
let _wrapperUnit = require('../units/:wrapper.unit');
...
let createContext = (width, height, options) => {
  let i, gl;
  ...
  let contextAttributes = new _webGLBinding.WebGLContextAttributes(flag(options, 'alpha', true), flag(options, 'depth', true), flag(options, 'stencil', false), false, // flag(options, 'antialias', true),
  flag(options, 'premultipliedAlpha', true), flag(options, 'preserveDrawingBuffer', false), flag(options, 'preferLowPowerToHighPerformance', false), flag(options, 'failIfMajorPerformanceCaveat', false));
  ...
  try {
    gl = new _webGLBinding.WebGLRenderingContext(1, 1, contextAttributes.alpha, contextAttributes.depth, contextAttributes.stencil, contextAttributes.antialias, contextAttributes.premultipliedAlpha, contextAttributes.preserveDrawingBuffer, contextAttributes.preferLowPowerToHighPerformance, contextAttributes.failIfMajorPerformanceCaveat);
  } catch (e) {
    throw e;
  }
  if (!gl) {
    throw ':WEBGL.UNIT.JS => Couldn\'t create WebGLRenderingContext. Your system may not support this function.';
  }
  ...
  let numTextures = gl.getParameter(gl.MAX_COMBINED_TEXTURE_IMAGE_UNITS);

  gl._textureUnits = new Array(numTextures);

  for (i = 0; i < numTextures; ++i) {
    gl._textureUnits[i] = new _webGLBinding.WebGLTextureUnit(i);
  }
  ...
  for (i = 0; i < numAttribs; ++i) {
    gl._vertexAttribs[i] = new _webGLBinding.WebGLVertexAttribute(gl, i);
  }
  ...
  _webGLBinding.allocateDrawingBuffer(gl, width, height);

  gl._attrib0Buffer = gl.createBuffer();
  ...
  return _wrapperUnit.wrapContext(gl);
};
