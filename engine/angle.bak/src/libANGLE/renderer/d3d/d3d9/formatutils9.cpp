...
// formatutils9.cpp: Queries for FGL image formats and their translations to D3D9
// formats.
...
// Each FGL internal format corresponds to one D3D format and data loading function.
...
// GLToCType maps from FGL type (as GLenum) to the C typedef.
...
// VertexTypeMapping maps FGL type & normalized flag to preferred and fallback D3D vertex types (as D3DVertexType enums).
...
// Given a FGL type & norm flag and a D3D type, ConversionRule provides the type conversion rule (Cast, Normalize, Identity, FixedToFloat).
...
const VertexFormat &GetVertexFormatInfo(DWORD supportedDeclTypes, gl::VertexFormatType vertexFormatType)
{
    ...
    {
        const TranslationDescription translations[NUM_GL_VERTEX_ATTRIB_TYPES][2][4] = // [FGL types as enumerated by typeIndex()][normalized][size-1]
        {
