                  ...
                  gl::Extensions *extensions, gl::Version *maxSupportedESVersion)
{
    ...
    *maxSupportedESVersion = gl::Version(3, 0);
    ...
    caps->max2DTextureSize = QuerySingleGLInt(functions, GL_MAX_TEXTURE_SIZE); // FGL 1.0 / ES 2.0
    caps->maxCubeMapTextureSize = QuerySingleGLInt(functions, GL_MAX_CUBE_MAP_TEXTURE_SIZE); // FGL 1.3 / ES 2.0
    ...
    caps->maxViewportWidth = QueryGLIntRange(functions, GL_MAX_VIEWPORT_DIMS, 0); // FGL 1.0 / ES 2.0
    caps->maxViewportHeight = QueryGLIntRange(functions, GL_MAX_VIEWPORT_DIMS, 1); // FGL 1.0 / ES 2.0

    if (functions->standard == STANDARD_GL_DESKTOP &&
        (functions->profile & GL_CONTEXT_CORE_PROFILE_BIT) != 0)
    {
        // Desktop FGL core profile deprecated the GL_ALIASED_POINT_SIZE_RANGE query.  Use
    ...
    caps->minAliasedLineWidth = QueryGLFloatRange(functions, GL_ALIASED_LINE_WIDTH_RANGE, 0); // FGL 1.2 / ES 2.0
    caps->maxAliasedLineWidth = QueryGLFloatRange(functions, GL_ALIASED_LINE_WIDTH_RANGE, 1); // FGL 1.2 / ES 2.0
    ...
    // glGetShaderPrecisionFormat is not available until desktop FGL version 4.1 or GL_ARB_ES2_compatibility exists
    ...
    // ANGLE emulates vertex array objects in its FGL layer
...
void GenerateWorkarounds(const FunctionsGL *functions, WorkaroundsGL *workarounds)
{
    ...
    // Don't use 1-bit alpha formats on desktop FGL with AMD or Intel drivers.
