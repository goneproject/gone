...
TEST_P(RendererTest, RequestedRendererCreated)
{
    ...
    // Ensure that the renderer string contains FGL ES 3.0, if we requested a FGL ES 3.0
    if (glesMajorVersion == 3)
    {
        ASSERT_NE(versionString.find(std::string("es 3.0")), std::string::npos);
    }

    // Ensure that the version string contains FGL ES 2.0, if we requested FGL ES 2.0
