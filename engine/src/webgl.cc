...
WebGLRenderingContext::WebGLRenderingContext(
        int width, int height, bool alpha, bool depth, bool stencil, bool antialias, bool premultipliedAlpha,
        bool preserveDrawingBuffer, bool preferLowPowerToHighPerformance, bool failIfMajorPerformanceCaveat) :
        state(GLCONTEXT_STATE_INIT), unpack_flip_y(false), unpack_premultiply_alpha(false),
        unpack_colorspace_conversion(0x9244), unpack_alignment(4), next(NULL), prev(NULL), lastError(GL_NO_ERROR) {
...
void WebGLRenderingContext::dispose() {
    ...
    // TODO: FIX
    // for (int iter; std::map < std::pair < GLuint, GLObjectType > ::iteratoriter = objects.begin(), iter != objects.end(); ++iter) {

    /*
     for (int iter = 0; std::map < std::pair < GLuint, iter != objects.end(); ++iter) {
    ...
    if (unpack_flip_y) {
        for (int i = 0, j = height - 1; j >= 0; ++i, --j) {
            memcpy(
                    ...
                    reinterpret_cast<void *>(pixels + i * rowStride), width * pixelSize);
        }
    } else {
        memcpy(
...
GL_METHOD(IsExtensionSupported) {
    GL_BOILERPLATE;
    ...
    //const char *extension = info[0].c_str()
    const char *extension = ToCString(info[0]->ToString());
    //GLenum result = glewContextIsSupported(extension);
    GLenum result = glewIsSupported(extension);
...
GL_METHOD(Hello) {
    GL_BOILERPLATE;
    info.GetReturnValue().Set(Nan::New<v8::String>("Hey!").ToLocalChecked());
}
